...
var timeService = {
    ...
    calculateUVT : function (timeFragments) {
        ...
        if (timeStamps.length % 2 !== 0){
            return {data: null, error: "An invalid number of timestamps were entered. There should be two timestamps for each viewed fragment."}
        }
        var result = this.parseTimeStamps(timeStamps);
        if (result.error){
            return {fragments: null, totalTime: null, error: result.error};
        }
        ...
        return {fragments: uniqueViewedFragments, totalTime: uniqueViewTime, error: null};
    },
    ...
    parseTimeStamps : function(timeStamps){
        ...
        while (i < timeStamps.length){
            ...
            if (isNaN(timeStamps[i]) || isNaN(timeStamps[i+1])){
                return {viewedFragments: fragments, error: "Only numeric characters may be entered for fragment time stamps."};
            }
            ...
            if (startTime > endTime){
                return {viewedFragments: fragments, error: "Time stamps must occur in a sequence of start time followed by end time. A fragment's start time may not be larger than its end time."};
            }

            // store time stamp fragment
            fragments.push({startTime : startTime, endTime : endTime, time: endTime-startTime});
            i += 2;
        }

        return {viewedFragments: fragments, error: null};
